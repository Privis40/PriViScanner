...
class ReconReport(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 8,
                  f"PriViSecurity  |  {TOOL} v{VERSION}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.ln(18)
...
def generate_pdf_report(report_data: dict, domain: str, operator: dict = None) -> str:
    ...
    try:
        ...
        pdf.cell(0, 6, f"  crt.sh Subdomains ({len(crt_subs)} found):", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_font(FF, "", 8)
        ...
        if crt_subs:
            ...
            for i in range(0, len(items), 2):
                ...
                pdf.cell(col_w, 5, _safe(f"  - {left[:42]}"), new_x=XPos.RIGHT, new_y=YPos.TOP)
                pdf.cell(0, 5, _safe(f"  - {right[:42]}") if right else "", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        pdf.set_font(FF, "B", 9)
        pdf.set_text_color(60, 60, 60)
        pdf.cell(0, 6, f"  Wayback Machine Endpoints ({len(wb_urls)} interesting):", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_font(FF, "", 7)
        pdf.set_text_color(0, 0, 0)
        for url in wb_urls[:25]:
            pdf.cell(0, 4, _safe(f"  • {url[:95]}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        pdf.cell(0, 6, f"  GitHub Code Exposure ({len(leaks)} result(s)):", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_font(FF, "", 8)
        pdf.set_text_color(0, 0, 0)
        for leak in leaks:
            pdf.cell(0, 5, _safe(f"  • [{leak['repo']}] {leak['file']}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font(FF, "I", 7)
        ...
        pdf.set_font(FF, "B", 9)
        pdf.set_text_color(60, 60, 60)
        pdf.cell(0, 6, f"  CMS Detected: {', '.join(cms_list) if cms_list else 'None'}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(2)
        ...
        def _pdf_path_table(title, items, cols, alert=False):
            ...
            for col, w in zip(cols, widths):
                pdf.cell(w, 6, f"  {col}", fill=True, new_x=XPos.RIGHT, new_y=YPos.TOP)
            ...
            for item in items[:30]:
                ...
                for col, w in zip(cols, widths):
                    ...
                    pdf.cell(w, 5, f"  {val[:max_chars]}", fill=True, new_x=XPos.RIGHT, new_y=YPos.TOP)
        ...
        if gql:
            ...
            pdf.cell(0, 6, _safe(f"  ⚠ GraphQL Introspection ENABLED at {gql['path']}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        else:
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, _safe("  ✔ GraphQL Introspection: Not detected"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if af.get("error_disclosure"):
            pdf.set_font(FF, "B", 9)
            pdf.set_text_color(196, 30, 58)
            pdf.cell(0, 6, "  Error / Stack Trace Disclosure:", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font(FF, "", 7)
            pdf.set_text_color(0, 0, 0)
            for e in af["error_disclosure"][:5]:
                pdf.cell(0, 4, _safe(f"  • {e['path']} (HTTP {e['status']})"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                pdf.set_font(FF, "I", 7)
                pdf.set_text_color(80, 80, 80)
                pdf.cell(0, 4, f"    {e.get('snippet','')[:90]}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                pdf.set_font(FF, "", 7)
                pdf.set_text_color(0, 0, 0)
        pdf.ln(4)
        ...
        if hits:
            ...
            pdf.set_text_color(196, 30, 58)
            pdf.cell(0, 7, _safe("  🔴 DEFAULT CREDENTIALS ACCEPTED:"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            ...
            pdf.set_font(FF, "", 9)
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, _safe("  ✔ No default credentials accepted."), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if jwt_issues:
            pdf.set_text_color(196, 30, 58)
            for j in jwt_issues:
                pdf.cell(0, 6, _safe(f"  ⚠ JWT Issue at {j['path']}: {j['issue']}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        else:
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, _safe("  ✔ No JWT vulnerabilities detected."), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if bypass_hints:
            pdf.set_font(FF, "B", 9)
            pdf.set_text_color(196, 30, 58)
            pdf.cell(0, 6, _safe(f"  ⚠ {len(bypass_hints)} Auth Bypass Hint(s) via Header Manipulation:"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font(FF, "", 8)
            pdf.set_text_color(0, 0, 0)
            for b in bypass_hints[:10]:
                pdf.cell(0, 5, _safe(f"  • {b['path']}  |  Header: {b['header']}  |  Status: {b['status']}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if session_issues:
            ...
            pdf.cell(0, 6, "  Session Cookie Issues:", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font(FF, "", 8)
            pdf.set_text_color(0, 0, 0)
            for s in session_issues:
                pdf.cell(0, 5, _safe(f"  • {s['path']}: {s['issue']}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(4)
        ...
        pdf.set_font(FF, "B", 9)
        pdf.set_text_color(60, 60, 60)
        pdf.cell(0, 6, f"  JS Files Scanned: {len(js_files)}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(1)
        if js_secrets:
            pdf.set_font(FF, "B", 9)
            pdf.set_text_color(196, 30, 58)
            pdf.cell(0, 6, _safe(f"  ⚠ {len(js_secrets)} Secret(s) / Credential(s) Found:"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font(FF, "B", 8)
            pdf.set_fill_color(26, 26, 46)
            pdf.set_text_color(255, 255, 255)
            ...
            pdf.set_font(FF, "", 9)
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, _safe("  ✔ No secrets or credentials found in JS files."), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if js_paths:
            pdf.set_font(FF, "B", 9)
            pdf.set_text_color(60, 60, 60)
            pdf.cell(0, 6, f"  Internal Paths Discovered ({len(js_paths)}):", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font(FF, "", 8)
            pdf.set_text_color(0, 0, 0)
            for p in js_paths[:30]:
                pdf.cell(0, 4, _safe(f"  • [{p['file'][:20]}] {p['path'][:70]}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(4)
        ...
        if cves:
            pdf.set_font(FF, "B", 9)
            pdf.set_text_color(196, 30, 58)
            pdf.cell(0, 6, _safe(f"  ⚠ {len(cves)} CVE(s) matched to detected technologies:"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.ln(1)
            pdf.set_font(FF, "B", 8)
            pdf.set_fill_color(26, 26, 46)
            pdf.set_text_color(255, 255, 255)
            ...
            pdf.set_font(FF, "I", 8)
            pdf.set_text_color(80, 80, 80)
            pdf.cell(0, 5, "  Full CVE details: https://nvd.nist.gov/vuln/detail/<CVE-ID>", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        else:
            pdf.set_font(FF, "", 9)
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, _safe("  ✔ No HIGH/CRITICAL CVEs matched to detected tech stack."), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if cert.get("valid"):
            ...
            if sans:
                pdf.set_font(FF, "B", 9)
                pdf.set_text_color(60, 60, 60)
                pdf.cell(0, 7, "  Subject Alternative Names (SANs):", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                pdf.set_font(FF, "", 8)
                pdf.set_text_color(0, 0, 0)
                for san in sans:
                    pdf.cell(0, 5, _safe(f"    • {san}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        else:
            pdf.set_font(FF, "I", 9)
            pdf.cell(0, 6, f"  {cert.get('error', 'Certificate inspection failed.')}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(4)
        ...
        if missing_hdrs or present_hdrs:
            ...
            if missing_hdrs:
                ...
                for h in missing_hdrs:
                    ...
                    pdf.set_text_color(*risk_color)
                    pdf.cell(0, 5, _safe(f"    ✘ [{h['risk']}] {h['header']}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                    pdf.set_font(FF, "I", 7)
                    pdf.set_text_color(80, 80, 80)
                    pdf.cell(0, 4, f"      → {h['impact'][:90]}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                    pdf.set_font(FF, "", 8)
            if present_hdrs:
                pdf.ln(2)
                pdf.set_font(FF, "B", 8)
                ...
                for h in present_hdrs:
                    pdf.set_text_color(30, 130, 30)
                    pdf.cell(0, 5, _safe(f"    ✔ {h['header']}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_text_color(0, 0, 0)
        else:
            pdf.set_font(FF, "I", 9)
            pdf.cell(0, 6, f"  {sh.get('error', 'Header check failed.')}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(4)
        ...
        if vulns:
            for v in vulns:
                ...
                pdf.set_font(FF, "B", 9)
                pdf.cell(0, 6,
                         f"  [FINDING] Port {v['port']}  -  {v['script_id']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                pdf.set_font(FF, "", 8)
                pdf.set_text_color(60, 60, 60)
