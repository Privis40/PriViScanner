    ...
    try:
        ...
        if crt_subs:
            ...
            for i in range(0, len(items), 2):
                ...
                pdf.cell(col_w, 5, f"  • {left[:42]}", new_x=XPos.RIGHT, new_y=YPos.TOP)
                pdf.cell(0, 5, f"  • {right[:42]}" if right else "",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        else:
        ...
        for url in wb_urls[:25]:
            pdf.cell(0, 4, f"  • {url[:95]}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        pdf.set_text_color(0, 0, 0)
        for leak in leaks:
            pdf.cell(0, 5, f"  • [{leak['repo']}] {leak['file']}",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if gql:
            ...
            pdf.cell(0, 6, f"  ⚠ GraphQL Introspection ENABLED at {gql['path']}",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        else:
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, "  ✔ GraphQL Introspection: Not detected",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_text_color(0, 0, 0)
        pdf.ln(2)
        ...
        if af.get("error_disclosure"):
            ...
            for e in af["error_disclosure"][:5]:
                pdf.cell(0, 4, f"  • {e['path']} (HTTP {e['status']})",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                pdf.set_font("Helvetica", "I", 7)
                pdf.set_text_color(80, 80, 80)
        ...
        if hits:
            ...
            pdf.set_text_color(196, 30, 58)
            pdf.cell(0, 7, "  🔴 DEFAULT CREDENTIALS ACCEPTED:",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font("Helvetica", "B", 8)
            ...
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, "  ✔ No default credentials accepted.",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_text_color(0, 0, 0)
        pdf.ln(2)
        ...
        if jwt_issues:
            pdf.set_text_color(196, 30, 58)
            for j in jwt_issues:
                pdf.cell(0, 6, f"  ⚠ JWT Issue at {j['path']}: {j['issue']}",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        else:
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, "  ✔ No JWT vulnerabilities detected.",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_text_color(0, 0, 0)
        pdf.ln(2)
        ...
        if bypass_hints:
            pdf.set_font("Helvetica", "B", 9)
            pdf.set_text_color(196, 30, 58)
            pdf.cell(0, 6, f"  ⚠ {len(bypass_hints)} Auth Bypass Hint(s) via Header Manipulation:",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font("Helvetica", "", 8)
            pdf.set_text_color(0, 0, 0)
            for b in bypass_hints[:10]:
                pdf.cell(0, 5, f"  • {b['path']}  |  Header: {b['header']}  |  Status: {b['status']}",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if js_secrets:
            pdf.set_font("Helvetica", "B", 9)
            pdf.set_text_color(196, 30, 58)
            pdf.cell(0, 6, f"  ⚠ {len(js_secrets)} Secret(s) / Credential(s) Found:",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font("Helvetica", "B", 8)
            pdf.set_fill_color(26, 26, 46)
            pdf.set_text_color(255, 255, 255)
            ...
            pdf.set_font("Helvetica", "", 9)
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, "  ✔ No secrets or credentials found in JS files.",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_text_color(0, 0, 0)
        pdf.ln(2)
        if js_paths:
            ...
            for p in js_paths[:30]:
                pdf.cell(0, 4, f"  • [{p['file'][:20]}] {p['path'][:70]}",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(4)
        ...
        if cves:
            pdf.set_font("Helvetica", "B", 9)
            pdf.set_text_color(196, 30, 58)
            pdf.cell(0, 6, f"  ⚠ {len(cves)} CVE(s) matched to detected technologies:",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.ln(1)
            pdf.set_font("Helvetica", "B", 8)
            pdf.set_fill_color(26, 26, 46)
            ...
            pdf.set_font("Helvetica", "", 9)
            pdf.set_text_color(30, 130, 30)
            pdf.cell(0, 6, "  ✔ No HIGH/CRITICAL CVEs matched to detected tech stack.",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_text_color(0, 0, 0)
        pdf.ln(4)
        ...
        if cert.get("valid"):
            ...
            if sans:
                ...
                pdf.set_text_color(0, 0, 0)
                for san in sans:
                    pdf.cell(0, 5, f"    • {san}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        else:
            pdf.set_font("Helvetica", "I", 9)
        ...
        if missing_hdrs or present_hdrs:
            ...
            if missing_hdrs:
                ...
                for h in missing_hdrs:
                    ...
                    pdf.set_text_color(*risk_color)
                    pdf.cell(0, 5, f"    ✘ [{h['risk']}] {h['header']}",
                             new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                    pdf.set_font("Helvetica", "I", 7)
                    pdf.set_text_color(80, 80, 80)
